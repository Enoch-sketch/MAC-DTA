...
def train(model, train_loader, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        l = l + loss.item()
        cl = cl + (cl_loss.item() if hasattr(cl_loss, 'item') else cl_loss)
        loss = (cl_loss if hasattr(cl_loss, 'item') else 0) + loss
        loss.backward()
        optimizer.step()
...
def get_ci(y, f):
    """Concordance Index — vectorised via sampling for large inputs."""
    ...
    if n * (n - 1) // 2 <= MAX_PAIRS:
        ...
        idx_i = rng.randint(0, n, size=MAX_PAIRS)
        idx_j = rng.randint(0, n, size=MAX_PAIRS)
        mask = idx_i != idx_j
    ...
    concordant = np.sign(dy) == np.sign(df)
    tied_pred = df == 0
    ci = (concordant.sum() + 0.5 * tied_pred.sum()) / len(dy)
    return float(ci)
